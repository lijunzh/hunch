...
impl PropertyMatcher for SubtitleLanguageMatcher {
    fn find_matches(&self, input: &str) -> Vec<MatchSpan> {
        ...
        if let Ok(Some(cap)) = SUB_LANG_EXT.captures(input) {
            if let Some(lang) = cap.name("lang") {
                if let Some(normalized) = normalize_language(lang.as_str()) {
                    matches.push(
                        MatchSpan::new(
                            lang.start(),
                            lang.end(),
        ...
        if let Ok(Some(cap)) = LANG_SUBFORCED.captures(input) {
            ...
            if let Some(lang) = cap.name("lang")
                .and_then(|l| normalize_language(l.as_str()))
            {
                if !matches.iter().any(|m| m.overlaps(&MatchSpan::new(full.start(), full.end(), Property::SubtitleLanguage, ""))) {
                    matches.push(
                        MatchSpan::new(
                            full.start(),
                            full.end(),
        ...
        if let Ok(Some(cap)) = SUB_LANG.captures(input) {
            if let Some(langs) = cap.name("langs") {
                let full = cap.get(0).unwrap();
                if !matches.iter().any(|m| m.overlaps(&MatchSpan::new(full.start(), full.end(), Property::SubtitleLanguage, ""))) {
        ...
        if matches.is_empty() {
            if let Ok(Some(cap)) = LANG_DASH_SUB.captures(input) {
                if let Some(lang) = cap.name("lang")
                    .and_then(|l| normalize_language(l.as_str()))
                {
                    let full = cap.get(0).unwrap();
        ...
        if matches.is_empty() {
            if let Ok(Some(cap)) = LEGENDADO.captures(input) {
                let full = cap.get(0).unwrap();
                let lang = cap.name("lang")
                    .and_then(|l| normalize_language(l.as_str()))
                    .unwrap_or("und");
        ...
        if matches.is_empty() {
            if let Ok(Some(cap)) = SUBTITULADO.captures(input) {
                let full = cap.get(0).unwrap();
                let lang = cap.name("lang")
                    .and_then(|l| normalize_language(l.as_str()))
                    .unwrap_or("und");
        ...
        if matches.is_empty() {
            if let Ok(Some(cap)) = GENERIC_SUB.captures(input) {
                let full = cap.get(0).unwrap();
